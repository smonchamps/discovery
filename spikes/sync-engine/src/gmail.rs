...
pub fn interesting_capabilities(session: &mut ImapSession) -> anyhow::Result<Vec<String>> {
    let caps = session.capabilities().context("lecture des capabilities")?;
    Ok(["CONDSTORE", "QRESYNC", "UIDPLUS", "IDLE", "X-GM-EXT-1"]
        .into_iter()
        .filter(|cap| caps.has_str(cap))
        .map(str::to_string)
        .collect())
}
