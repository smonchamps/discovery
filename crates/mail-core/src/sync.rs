...
mod tests {
    ...
    use crate::remote::MailboxSnapshot;

    /// Serveur simulé : rejoue ajouts, suppressions, flags, UIDVALIDITY,
    /// et journalise les lots demandés pour vérifier l'ordre de synchro.
    struct FakeServer {
        uid_validity: u32,
        condstore: bool,
        modseq: u64,
        messages: BTreeMap<Uid, (Envelope, u64)>,
        fetch_batches: Vec<Vec<Uid>>,
    }
    ...
    impl FakeServer {
        ...
        fn add(&mut self, uid: Uid, subject: &str) {
            self.modseq += 1;
        ...
        fn expunge(&mut self, uid: Uid) {
            self.messages.remove(&uid);
            self.modseq += 1;
        }

        fn mark_seen(&mut self, uid: Uid) {
            self.modseq += 1;
